#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Copyright (C) 2025 Xavier ZZ

__author__ = "Xavier ZZ"
__copyright__ = "Copyright 2025, Xavier ZZ"
__license__ = "GPL"
__version__ = "1.0"
...
with open("data/france.csv", mode="r") as csvfile:
...
class Family:
    ...
    def display_generation(self, gen):
        ...
        for p in gen:
            ...
            for i in range(0, int(n_house / 2)):
                h = p.households[i]
            ...
            for i in range(int(n_house / 2), n_house):
                h = p.households[i]
                spouse = Family.get_spouse(h, p)
        ...
        for p in gen:
            for h in p.households:
                ...
                print(
                    "\t\t"
                    + " -> ".join(map(lambda x: "h%d_%d" % (h.id, x), range(n_house)))
                    + ";"
                )
